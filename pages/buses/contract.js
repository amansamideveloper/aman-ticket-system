...
import { checkinUser, bookingContract, tryAnother, getcontractDeparture, getcontractDestination } from '../../actions/checkinAction'
...
function Daily(props) {
    ...
    useEffect(() => {
        props.getcontractDeparture()
        props.getcontractDestination()

    }, [])
    console.log(props.auth)
    const { data } = props.errors
    const error = props.daily.checkbus
    let success = props.daily.checkbus?.success
    ...
    function selectThisbus(data) {
        ...
        const bookingData = {
            ...
            total: data.bus?.seats * serviceCharge + data.price
        }
                        ...
                        <div className="items-center flex flex-wrap">
                            {bookingForm && <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                                <div className="text-blueGray-400 text-center mb-3 font-bold">
                                    <small> Fill all the requirement for contract Bus</small>
                                </div>
                            ...
                            {/* props.daily.checkbus.data */}
                            {props.daily.checkbus?.data.map((res, i) => {
                                ...
                                return res.available.length < 1 ? (
                                    <div key={i} className="w-full md:w-4/12 px-4 text-center cursor-pointer" onClick={() => selectThisbus(res)} >
                                        <div className={res._id === datafromselect?.id ? "relative flex flex-col min-w-0 break-words text-white bg-teal-500 w-full mb-8 shadow-lg rounded-lg" : "relative flex flex-col min-w-0 break-words bg-white w-full mb-8 shadow-lg rounded-lg"}>
                                            ...
                                            <div className="px-4 py-5 flex-auto">
                                                ...
                                                <h6 className="text-md font-semibold"> {res.price} birr/person</h6>

                                                <h6 className="text-md "> service charge 20birr/person</h6>
                                                <h6 className="text-md "> Total service charge {res.bus?.seats * serviceCharge} birr</h6>
                                                <h6 className="text-md ">Total : {res.bus?.seats * serviceCharge + res.price} birr</h6>
                                                <h6 className="text-md "> {res.bus?.name}</h6>

                                                <p className="mt-2 mb-4">Number of seats are {res.bus?.seats}
                                                </p>
                                    ...
                                    <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-8 shadow-lg rounded-lg">
                                        <div className="px-4 py-5 flex-auto">
                                            ...
                                            <h6 className="text-xl font-semibold">This bus already taken Another day</h6>
                                            <p className="mt-2 mb-4 text-blueGray-500">
                                        ...
                                        <form onSubmit={checkin}>
                                            ...
                                            <div className="relative w-full mb-3">
                                                ...
                                                <select onChange={e => setDeparture(e.target.value)} className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                                    ...
                                                    {props.contract && props.contract.departure && props.contract.departure.success?.map((item) => (
                                            ...
                                            <div className="relative w-full mb-3">
                                                ...
                                                <select onChange={e => setDestination(e.target.value)} className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                                    <option value=''>select</option>
                                                    {props.contract && props.contract.destination && props.contract.destination.success?.map((item) => (
                                                        <option value={item}>{item}</option>
                                                    ))}
                                                </select>
...
export default connect(mapStateToProps, { checkinUser, bookingContract, tryAnother, getcontractDeparture, getcontractDestination })(Daily)
